...
export function LookAtMeAvatar(props: AvatarOptions) {
  ...
  const { frames, frameBasePath, angleMap, size, width, height, objectFit, deadZone, tracking, alt } = props;
  useEffect(() => {
    let active = true;
    setError('');
    let avatar: ReturnType<typeof mountAvatar> | undefined;
    try {
      avatar = mountAvatar(root.current!, { frames, frameBasePath, angleMap, size, width, height, objectFit, deadZone, tracking, alt, onFrameChange: state => callbacks.current.onFrameChange?.(state) });
  ...
  }, [frames, frameBasePath, angleMap, size, width, height, objectFit, deadZone, tracking, alt]);
